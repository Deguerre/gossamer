...
namespace // anonymous
{
    ...
    public:
        ...
        void fill()
        {
            ...
            if (lhs.size() > 0)
            {
                ...
                JobManager::Token lt = mMgr.enqueue(bind(&Elem::fill, mLhs.get()), mLhs->deps());
                mDeps.insert(lt);
            }
            if (rhs.size() > 0)
            {
                ...
                JobManager::Token rt = mMgr.enqueue(bind(&Elem::fill, mRhs.get()), mRhs->deps());
                mDeps.insert(rt);
            }
        }
    ...
    void do_merge(JobManager& pMgr, ElemPtr& pRoot, const string& pBaseName, uint64_t pK, uint64_t pN, FileFactory& pFactory)
    {
        ...
        while (true)
        {
            JobManager::Token t = pMgr.enqueue(bind(&Elem::fill, pRoot.get()), pRoot->deps());
            pMgr.wait(t);
